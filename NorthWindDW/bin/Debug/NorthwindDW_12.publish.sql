...
GO
PRINT N'Quitando Clave externa [dbo].[FK_DimCustomer]...';


GO
ALTER TABLE [dbo].[FactOrders] DROP CONSTRAINT [FK_DimCustomer];


GO
PRINT N'Quitando Clave externa [dbo].[FK_DimProduct]...';


GO
ALTER TABLE [dbo].[FactOrders] DROP CONSTRAINT [FK_DimProduct];


GO
PRINT N'Quitando Clave externa [dbo].[FK_DimEmployee]...';


GO
ALTER TABLE [dbo].[FactOrders] DROP CONSTRAINT [FK_DimEmployee];
...
CREATE TABLE [dbo].[tmp_ms_xx_FactOrders] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PK_FactOrders1] PRIMARY KEY CLUSTERED ([OrderID] ASC, [ProductSK] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[FactOrders])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_FactOrders] ([OrderID], [ProductSK], [CustomerSK], [EmployeeSK], [UnitPrice], [Quantity], [Discount], [OrderDateKey], [RequiredDateKey], [ShippedDateKey], [Freight], [ShipName], [ShipAddress], [ShipCity], [ShipRegion], [ShipPostalCode], [ShipCountry], [ShipperCompanyName], [ShipperPhone], [OrderDate], [RequiredDate], [ShippedDate])
        SELECT   [OrderID],
                 [ProductSK],
                 [CustomerSK],
                 [EmployeeSK],
                 ...
                 [ShippedDate]
        FROM     [dbo].[FactOrders]
        ORDER BY [OrderID] ASC, [ProductSK] ASC;
    END
